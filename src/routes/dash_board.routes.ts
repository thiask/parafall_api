...
router_dash_board.get('/estatistics/vendas/:dataInicial/:dataFinal/:idCliente', async (req, res) => {
    ...
    const sql = `SELECT count(itens_vendas.id) as qtdVendida, sum(vendas.valor) as totalVendido, produtos.descricao FROM vendas
    ...
    WHERE vendas.createdAt BETWEEN '${req.params.dataInicial}' AND '${req.params.dataFinal}' ${getIdClient()}
